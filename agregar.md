...
La descarga es un archivo **Excel (.xlsx)** con las columnas requeridas.
...
|------------------|------------------|----------------------------------------------------|------------------------|---------------------------------------------------------------------------|
| marca            | Sí               | Valor del **controlador** (ver catálogo abajo)     | allied_telesis_awplus  | Debe coincidir exactamente con los valores de marca soportados.          |
| nombre           | Sí               | Texto (1–80)                                       | SW-NOMBRE 1            | Nombre único descriptivo.                                                |
| ip               | Sí               | IPv4 `A.B.C.D`                                     | 10.200.0.1             | No repitas IP.                                                            |
...
| snmp_community   | Sí               | Texto *(para SNMPv1/v2c)*                          | public                 | Para **v3**, deja `snmp_community` y configura credenciales v3 después.  |
...
| latitud          | No               | Decimal con **punto**                              | 25.6866                | Útil para mapas. Deja **NULL** si no aplica.                             |
| longitud         | No               | Decimal con **punto**                              | -100.3161              | Útil para mapas. Deja **NULL** si no aplica.                             |
...
| ruijie_os                  | Ruijie                        |
| sophos_sfos                | Sophos (compatibles)          |
...
> ⚠️ **Importante:** escribe el valor **exacto**, en **minúsculas** y **sin espacios**.  
...
allied_telesis_awplus,SW-NOMBRE 1,10.200.0.1,Offline,admin,admin#,Cuarto Telecom,26,2c,public,SI,1,NULL,NULL
ruckus_fastiron,SW-NOMBRE 3,10.200.0.3,Offline,admin,admin#,Cuarto Telecom,26,2c,public,SI,1,NULL,NULL
fortinet,SW-NOMBRE 4,10.200.0.4,Offline,admin,admin#,Cuarto Telecom,26,2c,public,NO,1,NULL,NULL
tplink_jetstream,SW-NOMBRE 10,10.200.0.10,Offline,admin,admin#,Cuarto Telecom,26,2c,public,NO,1,NULL,NULL
```

---

## 5️⃣ Subir el Excel
1. En la pantalla de carga, pulsa **“Examinar…”** y elige tu archivo.  
2. Haz clic en **“Cargar Excel”**.  
3. Revisa el resultado: el sistema **confirmará** los equipos creados o **indicará errores por fila**.
